...
(function (root, factory) {
    ...
    var select = function (query, context, filter) {
        ...
        if(quickId.test(query)) { //#id
            ...
            if(cls) { //tag.class  .clsss
                ret = toArray(getElementsByClass(cls, tag, context));
            } else if(tag) {
                ret = toArray(getElementsByTag(tag, context)); //tag
                if(id) { //tag#id
                    ret = filter(ret, function (elem) {
                        return elem.id == id;
                    });
                }
            } else if(id) { //#id #id
    ...
    function filterParents(parts, nodes, context, direct) {
        ...
        while((node = nodes[++i])) {
            p = node.parentNode;
            do {
                match = (!tag || tag === '*' || tag === p.nodeName) && (!id || p.id === id) && (!cls || RegExp('(^|\\s)' + cls + '(\\s|$)')
                    .test(p.className));
        ...
        (function () {
            ...
            select = function (query, context, filter) {
                ...
                if(context.nodeType === 9) {
                    ret = toArray(context.querySelectorAll(query));
                    if(filter) {
                        ret = filter(ret, filter);
                    }
                    return ret;
